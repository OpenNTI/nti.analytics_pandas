...
class QueryForumsCreated(TableQueryMixin):
	...
	def filter_by_period_of_time_and_course_id(self, start_date=None, end_date=None, course_id=()):
		...
		query = self.session.query(tc.timestamp,
								   ...
								   tc.forum_id,
								   tc.deleted,
								   tc.entity_root_context_id).filter(tc.timestamp.between(start_date, end_date)).filter(tc.root_context_id.in_(course_id))
		dataframe = orm_dataframe(query, self.columns)
		return dataframe
...
class QueryForumsCommentsCreated(TableQueryMixin):
	...
	def filter_by_period_of_time(self, start_date=None, end_date=None):
		...
		query = self.session.query(fcc.timestamp,
								   fcc.root_context_id.label('course_id'),
								   fcc.user_id,
								   fcc.topic_id,
								   fcc.session_id,
								   fcc.favorite_count,
	...
	def filter_by_period_of_time_and_course_id(self, start_date=None, end_date=None, course_id=()):
		...
		query = self.session.query(fcc.timestamp,
								   ...
								   fcc.is_flagged,
								   fcc.deleted,
								   fcc.root_context_id.label('course_id')).filter(fcc.timestamp.between(start_date, end_date)).filter(fcc.root_context_id.in_(course_id))
		dataframe = orm_dataframe(query, self.columns)
		return dataframe
...
class QueryForumCommentFavorites(TableQueryMixin):
	...
	def filter_by_period_of_time(self, start_date=None, end_date=None):
		...
		query = self.session.query(fcf.timestamp,
								   ...
								   fcf.comment_id,
								   fcf.creator_id,
								   fcf.root_context_id.label('course_id')).filter(fcf.timestamp.between(start_date, end_date))
		dataframe = orm_dataframe(query, self.columns)
		return dataframe
	...
	def filter_by_period_of_time_and_course_id(self, start_date=None, end_date=None, course_id=()):
		...
		query = self.session.query(fcf.timestamp,
								   ...
								   fcf.comment_id,
								   fcf.creator_id,
								   fcf.root_context_id.label('course_id')).filter(fcf.timestamp.between(start_date, end_date)).filter(fcf.root_context_id.in_(course_id))
		dataframe = orm_dataframe(query, self.columns)
		return dataframe
...
class QueryForumCommentLikes(TableQueryMixin):
	...
	def filter_by_period_of_time(self, start_date=None, end_date=None):
		...
		query = self.session.query(fcl.timestamp,
								   ...
								   fcl.comment_id,
								   fcl.creator_id,
								   fcl.root_context_id.label('course_id')).filter(fcl.timestamp.between(start_date, end_date))
		dataframe = orm_dataframe(query, self.columns)
		return dataframe
	...
	def filter_by_period_of_time_and_course_id(self, start_date=None, end_date=None, course_id=()):
		...
		query = self.session.query(fcl.timestamp,
								   ...
								   fcl.comment_id,
								   fcl.creator_id,
								   fcl.root_context_id.label('course_id')).filter(fcl.timestamp.between(start_date, end_date)).filter(fcl.root_context_id.in_(course_id))
		dataframe = orm_dataframe(query, self.columns)
		return dataframe
