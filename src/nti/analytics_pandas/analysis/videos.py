...
from nti.analytics_pandas.queries import QueryVideoEvents
...
class VideoEventsTimeseries(object):
    ...
    def __init__(self, session, start_date, end_date, course_id=None,
        ...
        if isinstance(course_id, Sequence):
        ...
        if not self.dataframe.empty:
            ...
            self.dataframe = cast_columns_as_category_(self.dataframe,
                                                       categorical_columns)
    ...
    def analyze_video_events(self, video_event_type=None):
        ...
        if video_event_type is None:
            ...
            if self.dataframe.empty:
                return
            dataframe = self.dataframe[
                ['timestamp_period', 'video_view_id',
                 'user_id', 'video_event_type']
            ]
            dataframe = dataframe.loc[
                dataframe['video_event_type'] == video_event_type
            ]
    ...
    def analyze_video_events_per_course_sections(self, video_event_type=u'WATCH'):
        """
        ...
        if self.dataframe.empty:
            return
        dataframe = self.dataframe[
            ['timestamp_period', 'video_view_id', 'user_id',
    ...
    def build_dataframe(self, dataframe, group_by_items):
        ...
        if df is not None:
            df.rename(columns={'user_id': 'number_of_unique_users',
                      ...
                      inplace=True)
            unique_users = df['number_of_unique_users']
            df['ratio'] = df['number_of_video_events'] / unique_users
        return df
