...
from nti.analytics_pandas.reports.views.notes import View
from nti.analytics_pandas.reports.views.notes import Context
...
class TestNoteEvents(AnalyticsPandasTestBase):
	...
	def test_generate_pdf_from_rml(self):
		...
		pdf_stream = rml2pdf.parseString(rml)
		pdf_stream.seek(0)
		fd = open('test_notes.pdf', 'w')
		shutil.copyfileobj(pdf_stream, fd)
		pdf_stream.close()
		assert_that(os.path.exists('test_notes.pdf'), is_(True))
